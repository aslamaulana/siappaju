...
use App\Models\User\User\Model_users;
use App\Models\User\User\Model_bidang;
use App\Models\User\User\Model_groups_users;
...
class Users extends BaseController
{
	...
	public function user()
	{
		if (has_permission('User')) :
			$users = $this->users->Users2();
			$data = [
				'gr' => 'opd',
				'mn' => 'bidang',
				'title' => 'User | OPD',
				'lok' => '<a href="/user/user/bidang">OPD</a> -> <b>User OPD</b>',
				'users' => $users,
	 ...
	 * ---------------------------------------------------
	 */
	public function users_add($level = '1', $id_user = '')
	{
		if (has_permission('User')) :
			$users = date("YmdHis");
			$data = [
			...
			return view('user/User/users_add', $data);
		else :
			throw \CodeIgniter\Exceptions\PageNotFoundException::forPageNotFound();
		endif;
	}
	...
	public function users_create()
	{
		...
		$this->users->save([
			'id' => user()->opd_id . buka($this->request->getVar('id')),
		...
		$this->group_users->save([
			'group_id' =>  user()->opd_id, /* akses adalah opd_id */
			'user_id' => user()->opd_id . buka($this->request->getVar('id')),
		]);


		session()->setFlashdata('pesan', 'Data berhasil di simpan.');
		return redirect()->to(base_url() . '/user/user/users/user/' . $this->request->getVar('akses'));
	}
	/*
	 * ---------------------------------------------------
	 * Ubah user
	 ...
	 * ---------------------------------------------------
	 */
	public function users_edit($id, $level = '1')
	{
		if (has_permission('User')) :
			...
			$data = [
				...
				'mn' => 'bidang',
				'title' => 'User | OPD',
				'lok' => 'OPD -> <a href="/user/user/users/user/">User OPD</a> -> <b>Ubah User OPD</b>',
				'validation' => \Config\Services::validation(),
				'users' => $users,
				'level' => $level,
			];
			return view('user/User/users_edit', $data);
		else :
			throw \CodeIgniter\Exceptions\PageNotFoundException::forPageNotFound();
		endif;
	}
	...
	public function ubah_password()
	{
		if (has_permission('User')) :
			...
			return view('user/User/password_edit', $data);
		else :
			throw \CodeIgniter\Exceptions\PageNotFoundException::forPageNotFound();
		endif;
	}
