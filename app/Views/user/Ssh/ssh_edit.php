...
<form action="<?= base_url('/user/ssh/ssh_pengajuan/ssh_update') ?>" method="POST" id="myForm">
	<div class="card-body">
		<?= csrf_field() ?>
		<div class="row">
				...
				<div class="row">
					<div class="col">
						<div class="form-group">
							...
							<div class="input-group">
								<textarea name="spesifikasi" class="form-control" maxlength="500" style="height: 125px;" required><?= $ssh['spesifikasi']; ?></textarea>
							</div>
						</div>
					</div>
				</div>
		...
		<div class="row">
			<div class="col">
				<table class="table table-bordered display nowrap table-sm" cellspacing="0" id="itemsTable">
					...
					<tbody>
						<?php $nomer = 1; ?>
						<?php foreach ($ssh_rekening as $row) : ?>
							<tr id="DeleteDb_<?= $row['id_ssh_rekening']; ?>">
								<td class="text-center"><?= $nomer++; ?></td>
								<td><?= "[" . $row['kode_rekening_rincian_objek_sub'] . "] " . $row['rekening_rincian_objek_sub']; ?></td>
								<td><button type="button" class="btn btn-danger" onclick="DeleteDb('<?= $row['id_ssh_rekening']; ?>')">Hapus</button></td>
								<input type="hidden" name="id[]" value="<?= $row['id_ssh_rekening']; ?>">
								<input type="hidden" name="rekening[]" value="<?= $row['rekening_rincian_objek_sub_id']; ?>">
							</tr>
						<?php endforeach ?>
					</tbody>
				</table>
...
<script>
	...
	function DeleteDb(id) {
		if (confirm("Apakah Anda yakin ingin menghapus data ini?")) {
			fetch("<?= base_url('/user/ssh/ssh_pengajuan/hapus_rekening'); ?>", {
					method: 'POST',
					headers: {
	...
	$(document).ready(function() {
		...
		// Memicu perubahan secara otomatis saat halaman dimuat
