							...
							<div class="input-group">
								<select name="sub_rincian_objek" id="sub_rincian_objek_id" class="form-control select2bs4 <?= ($validation->hasError('sub_rincian_objek')) ? 'is-invalid' : ''; ?>" required>
									<?php foreach ($akun as $row) : ?>
										<option value="<?= $row['id_jenis_rincian_objek_sub']; ?>" <?= $row['id_jenis_rincian_objek_sub'] == $hspk['jenis_rincian_objek_sub_id'] ? 'selected' : ''; ?>><?= '[' . $row['kode_jenis_rincian_objek_sub'] . '] ' . $row['jenis_rincian_objek_sub'] . ' [' . $row['kelompok_id'] . ' ]'; ?></option>
									<?php endforeach ?>
								</select>
